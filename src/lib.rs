...
const GROUP_LOG: u32 = 4;
const GROUP_CHUNKS: usize = 1 << GROUP_LOG;
const GROUP_SIZE: usize = CHUNK_SIZE * GROUP_CHUNKS;
use blake3::Hash;
...
pub(crate) struct ChunkGroupState {
    current_chunk: u64,
    leaf_hashes: [blake3::Hash; GROUP_CHUNKS - 1],
...
impl ChunkGroupState {
    ...
    pub fn len(&self) -> usize {
        self.leaf_hash_count * CHUNK_SIZE + self.current.len()
    }
    ...
    pub fn update(&mut self, input: &[u8]) -> &mut Self {
        ...
        while self.current.len() + input.len() > CHUNK_SIZE {
            ...
            self.leaf_hashes[self.leaf_hash_count] = self.current.finalize(false);
            self.leaf_hash_count += 1;
    ...
    pub fn finalize(&self, is_root: bool) -> Hash {
        if self.leaf_hash_count == 0 {
            ...
            let n = self.leaf_hash_count;
            leaf_hashes[..n].copy_from_slice(&self.leaf_hashes[..self.leaf_hash_count]);
